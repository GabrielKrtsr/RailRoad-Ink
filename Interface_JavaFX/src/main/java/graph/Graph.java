 ...
 */
public class Graph<V, E extends Edge<V>> implements Cloneable {
    ...
    public Graph<V, E> clone() {
        try {
            Graph<V, E> cloned = (Graph<V, E>) super.clone();
            /*
            cloned.adjacencyList.clear();
            for (Map.Entry<V, List<E>> entry : this.adjacencyList.entrySet()) {
                V vertex = entry.getKey();
                List<E> edges = new ArrayList<>(entry.getValue());
                cloned.adjacencyList.put(vertex, edges);
            }
             */
            return cloned;
    ...
    public void removeEdge(V nodeSet, V otherNodeSet) {
        List<E> edges = adjacencyList.get(nodeSet);
        if (edges != null) {
            edges.removeIf(edge -> edge.getTarget().equals(otherNodeSet));
        }
    }
}
